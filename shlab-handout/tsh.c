 ...
 *
 * e.g. fg %1 -> put job 1 to foreground to execute
 */
void do_bgfg(char **argv)
{
    char *id_str = argv[1];
    ...
    // sanity check for job id and pid
    char *endptr;
    ...
    {
        // job ID
        int jid = (int)strtol(id_str + 1, &endptr, 10);
        if (*endptr != '\0')
        {
    ...
    {
        // pid
        pid_t pid = (int)strtol(id_str, &endptr, 10);
        // sanity check
        if (*endptr != '\0')
        {
            printf("%s: argument must be a PID or %%jobid\n", id_str);
    ...
    // Here job is a valid pointer
    // 1. send it a SIGCONT signal
